...
class TestUrlsPost(TestCase):
    ...
    def setUp(self):
        ...
        self.l_tag = models.LocalTag.objects.create(title='Phone', global_tag=self.g_tag)
        self.l_tag.save()
        self.item = models.Item.objects.create(title='Phone', description='iphone11', slug='phone', tag=self.l_tag,
                                               user_id=user.id)
        self.item.save()

    def test_create_item_post(self):
        response = self.client.post('/create-item/', {'title': 'new phone', 'description': 'iphone10',
                                                      'tag': self.l_tag.id})  # self.l_tag have vlaue 9
    ...
    def test_update_item_post(self):
        response = self.client.post('/item/phone/update/',
                                    {'title': 'old phone', 'description': 'iphone7', 'tag': self.l_tag.id})
